...
def parse_hl7(hl7_message):
    ...
    for line in lines:
        if line.startswith("PID"):
            ...
            if pid_5_match:
                pid_5 = pid_5_match.group(1).strip()
            if pid_3_match:
                pid_3 = pid_3_match.group(1).strip()
            if pid_7_match:
                pid_7 = pid_7_match.group(1).strip()
        if line.startswith("OBR"):
            obr_3_match = re.search(r"(?:\|[^|]*){2}\|([^|]*)", line)
            obr_4_match = re.search(r"(?:\|[^|]*){3}\|([^|]*)", line)
            if obr_3_match:
                obr_3 = obr_3_match.group(1).strip()
            if obr_4_match:
        ...
        if line.startswith("OBX"):
            ...
            if obx_11_match:
                obx_11 = obx_11_match.group(1).strip()
            parts = line.split("|")
            if len(parts) > 5:
...
def process_hl7_file(hl7_file_path):
    ...
    try:
        ...
        if not all([pid_5, pid_3, pid_7, obr_3, obr_4_2, obx_11]):
            missing = []
            if not pid_5:
                missing.append("PID-5 (Patient Name)")
            if not pid_3:
                missing.append("PID-3 (Patient ID)")
            if not pid_7:
                missing.append("PID-7 (Patient DOB)")
            if not obr_3:
                missing.append("OBR-3 (Accession Number)")
            if not obr_4_2:
                missing.append("OBR-4-2 (Modality)")
            if not obx_11:
                missing.append("OBX-11")
        ...
        try:
            subprocess.run(img2dcm_command, check=True, capture_output=True, text=True)
        ...
        log.error(
            "Context: PID-5=%s PID-3=%s PID-7=%s OBR-3=%s OBR-4-2=%s OBX-11=%s",
            pid_5,
            pid_3,
            pid_7,
            obr_3,
